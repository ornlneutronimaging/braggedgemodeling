...
class MarchDollase(XtalOriProbModel):
    ...
    "Imaging of a spatial distribution of preferred orientation of crystallites
    by pulsed neutron Bragg edge transmission"
    by H Sato et. al.
    ...
    def __init__(self, r=None, beta=None):
        self.r = r or self.Texture(1.0)
        self.beta = beta or self.Texture(0.0)
        return
    ...
    def __call__(self, peak, wavelen):
        ...
        if np.isclose(r, 1.0):
            return 1.0
        alpha = np.pi / 2 - np.arcsin(wavelen / 2.0 / peak.d)
        beta = self.beta(hkl)
        delta = np.pi / 100.0
        phi = np.arange(-np.pi / 2, np.pi / 2 - delta / 2.0, delta)
        # array
        if isinstance(wavelen, np.ndarray):
            integral = (r * r - 1.0 / r) * (
                (np.cos(alpha) * np.cos(beta))[:, np.newaxis]
                - (np.sin(alpha) * np.sin(beta))[:, np.newaxis] * np.sin(phi)
            ) ** 2
            integral += 1.0 / r
            integral = 1.0 / (np.sqrt(integral) * integral)
            ret = np.sum(integral, axis=-1) * delta / np.pi
            ret[ret != ret] = 0
            return ret
        # number
        integral = (r * r - 1.0 / r) * (np.cos(alpha) * np.cos(beta) - np.sin(alpha) * np.sin(beta) * np.sin(phi)) ** 2
        integral += 1.0 / r
        integral = 1.0 / (np.sqrt(integral) * integral)
