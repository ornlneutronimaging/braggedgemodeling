...
def compute(
    ...
    """
    tolerance = 90.0 / N_RD / 2.0  # 90./36/2.
    ...
    peaks = list(iter_peaks(structure, T=300.0, max_index=max_hkl_index))
    ...
    # d_spacing_list = [2.07793, 1.800787, 1.272775, 1.085376, 1.038812, 0.90032,
    #                    0.825621, 0.804956, 0.734553, 0.692623, 0.692623, 0.6356, 0.6082,]
    d_spacing_list = [p.d for p in peaks]
    print(d_spacing_list)
    #
    outstream = open(out, "wt")
    hkls_outstream = open(hkls_out, "wt")
    matched_min = np.cos(tolerance * np.pi / 180.0)

    RDs = np.arange(N_RD + 1) * np.pi / 2.0 / N_RD  # from 0 to pi/2
    ...
    for ihkl, (hkl, d_spacing) in enumerate(zip(hkls, d_spacing_list)):
        print(ihkl)
        lambdas = 2.0 * d_spacing * np.sin(RDs)
        alfas = np.arccos(lambdas / 2.0 / d_spacing)
...
def isDuplicate(hkl, collection):
    if hkl in collection:
        return True
    h, k, l = hkl
    nhkl = -h, -k, -l
    if nhkl in collection:
        return True
    return False
