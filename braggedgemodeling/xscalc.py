...
class XSCalculator:
    ...
    def xs_coh_el(self, wavelen):
        ...
        vs = [
            ...
            * p.mult
            * self.xopm(p, wavelen)
            * self.extinction_factor(wavelen, p)
            * (p.d * 2 > wavelen)
            for p in self.diffpeaks
        ]
    ...
    def xs_coh_el__peak(self, wavelen, peak):
        ...
        v = (
            ...
            * peak.d
            * peak.mult
            * self.xopm(peak, wavelen)
            * self.extinction_factor(wavelen, peak)
            * (peak.d * 2 > wavelen)
        )
    ...
    def extinction_factor(self, wavelen, pk):
        ...
        if size == 0:
            return 1.0
        sin_theta = wavelen / 2.0 / pk.d
        sin_theta_2 = sin_theta**2
        cos_theta_2 = 1 - sin_theta_2
        x = size * size * (wavelen * 1e-10 * pk.F * 1e-15 / self.uc_vol * 1e30) ** 2
        EB = 1 / np.sqrt(1 + x)
        EL = (1 - x / 2 + x * x / 4 - 5 * x**3 / 48) * (x <= 1) + (
            np.sqrt(2 / np.pi / x) * (1 - 1 / 8.0 / x - 3.0 / 128 / x / x - 15.0 / 1024 / x**3)
        ) * (x > 1)
