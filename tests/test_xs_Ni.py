...
def _test_Fe():
    # NOTE: `laz` is never imported and "Fe.laz" does not exist; this helper is
    # disabled (leading underscore => pytest never collects it) and abandoned.
    d = laz.read("Fe.laz")  # noqa: F821
    peaks = [xscalc.DiffrPeak(hkl, F, d, mult) for hkl, F, d, mult in zip(d["hkl"], d["F"], d["d"], d["mult"])]
