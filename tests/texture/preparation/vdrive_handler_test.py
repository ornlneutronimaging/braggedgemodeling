...
class TestVDriveHandler(unittest.TestCase):
    ...
    def test_sin_omega(self):
        ...
        sin_omega_bank1_expected = [np.sin(np.pi * _omega / 180.0) for _omega in omega_bank1]
        ...
        sin_omega_bank2_expected = [np.sin(np.pi * _omega / 180.0) for _omega in omega_bank2]
    ...
    def test_mean_omega_45(self):
        ...
        mean_iv_omega_45_expected = [2.451589167, 2.399450833, 0.841268417, 1.271379167, 0.320264833, 0.288333667]
        ...
        mean_eiv_omega_45_expected = [0.054312417, 0.053232583, 0.01754925, 0.01939325, 0.007362275, 0.00547845]
    ...
    def test_stdev_omega_45(self):
        ...
        std_iv_omega_45_expected = [0.260401269, 0.332260911, 0.057889314, 0.122683245, 0.031568719, 0.040697648]
    ...
    def test_bank2_iv_ratio_omega_90(self):
        ...
        _mean_iv_ratio_omega_90_expected = [
            0.85891518,
            0.857602759,
            0.885132385,
            0.891429027,
    ...
    def test_output_table(self):
        ...
        _returned_expected = zip(output_table_calculated[0:21, 0], output_table_col0_top_expected)
        for _returned, _expected in _returned_expected:
            if np.isnan(_returned) and np.isnan(_expected):
                self.assertTrue(True)
            else:
        ...
        _returned_expected = zip(output_table_calculated[-21:, 0], output_table_col0_bottom_expected)
        for _returned, _expected in _returned_expected:
            if np.isnan(_returned) and np.isnan(_expected):
                self.assertTrue(True)
            else:
